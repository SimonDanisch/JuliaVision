...
@testset "RIFERunner: a fresh process compiles no pipelines" begin
    if !RIFERunner.ready()
        @info "no RIFERunner export; skipping the latency test"
    else
        script = tempname() * ".jl"
        ...
        elseif i === nothing
            ...
            @info "RIFE, first call in a fresh process" r
