...
using Test, FluxKleinRunner

@testset "FluxKleinRunner" begin
    ...
    if FluxKleinRunner.ready()
        @info "FLUX.2-klein-4B: export present"
        g = FluxKleinRunner.fluxkleingraph()
        @test g !== nothing
        w = FluxKleinRunner.fluxkleinweights()
        @test !isempty(w)
    else
        @info "FLUX.2-klein-4B: no export; run tools/export_fluxklein.py"
        ...
        @test_throws ErrorException FluxKleinRunner.fluxkleingraph()
    end
end
