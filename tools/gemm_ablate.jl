...
function run_abl(C, A, B, M, N, K, c, abl)
    kern = Lava.GEMM_STAGED_ABL_KERNELS[c]
    wg = Lava.gemm_wg(c)
    kern(BACKEND, wg)(C, A, B, nothing, identity,
                      Val(M), Val(N), Val(K), Val(abl);
                      ndrange = (M ÷ Lava.gemm_bm(c)) * (N ÷ Lava.gemm_bn(c)) * wg)
    C
end
...
function ablate(M, N, K, share)
    ...
    for abl in (:all, :nostage)
        ...
        for k in keys(pipes())
            ...
            s = Lava.pipeline_exec_stats(pipes()[k]); s === nothing && continue
    ...
    @printf("\n%d x %d x %d  (%.1f%% of the encoder's GEMM, tiling %dx%d/%dw)\n",
            M, N, K, share, Lava.gemm_bm(c), Lava.gemm_bn(c), Lava.gemm_wg(c) ÷ 32)
