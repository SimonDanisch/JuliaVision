...
using DNNKernels: loadgraph, execute!, readsafetensors, toback, fusableset, Workspace
...
function timed(f, backend; n = 9, reps = 3)
    ...
    # ramp, and a median over it lands anywhere in a 5x band, which is what
    # run-to-run "noise" on a small graph is. Costly only where a call is cheap,
